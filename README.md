...
If there is ever an issue with running containers because a port was already allocated (i.e. ``port 3307`` was already allocated to an existing container), then simply go through the directories ``Milestone1`` and ``Milestone2`` and perform ``docker compose down``. Then run ``docker compose up --build`` in ``FinalProject``. **This will take a while to build, so be patient!** Once all containers are running, type ``localhost`` in the search bar of your browser.
